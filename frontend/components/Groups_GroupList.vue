<template lang="pug">
div
	ModalForm_GroupInfo(id="form_changeGroup" v-bind:group_prop="selectedGroup" v-bind:allStudents="allStudents" v-bind:isReadonlyMode="true")
			...
			.card-body.p-0
				button(@click="onSelectClick(group)"
					...
					div(v-else)
						//on big devices
						div.hide_on_small
							.row.d-flex.justify-content-between.align-items-center.align-text-baseline.p-3
								h6.col {{group.name }}
								span.mr-2.ml-auto(v-if="group.students.length>0").badge.badge-secondary {{group.students.length}}
...
export default {
	...
	methods: {
		onSelectClick(group) {
			...
			this.$emit('onSelectChange', group)
		}
	}
}
</script>
